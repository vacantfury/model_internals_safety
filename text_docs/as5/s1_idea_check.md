...
> **Amended 2026-08-02 writing the pilot** (implementation: `scripts/phase0_regime_map.py`, `probes/linear.py`), on the sentence above that says the four measurements are taken "for every (model, family, prompt)".
>
> *Two of the four are probes, and a probe has no per-prompt verdict to give.* Ability and behaviour are measured on one prompt each; deployment and recognition are AUROCs over a population. That gap has to be closed explicitly, because the coherence constraint is checked cell by cell and mixing granularities manufactures instrument failures out of nothing — a family with 60% ability and one family-level deployment flag would report 40% of its cells as *deployment ✓ / ability ✗*, i.e. as the very instrument failure the constraint exists to detect. The pilot therefore reads each probe twice: the population licenses the cell (unless a (layer, position) cell separates above its shuffled-label control, every prompt in that family reads negative), and the individual prompt is then read at that cell against the **same-condition negative class** — same condition, so the encoding and its attack-template wrapper, which shift both classes together, cannot decide the label. Deployment's per-example scores are out-of-sample by construction (the probe is fit on the plain condition); recognition's are cross-validated, since that probe is fit inside the condition it reads. This construction is ours, not inherited — Arditi et al. and Zhao et al. report population separation and never need a per-prompt label — so it is carried as an open decision (`project_structure.md` §7, decision 6) and the pilot reports the underlying curves alongside the labels, so the paper can be written either way from one run.
>
> *One consequence worth stating in the paper.* The per-prompt reading has an operating point (the percentile of the negative distribution a prompt must beat), and at the default of 50 the benign control reads positive half the time **by construction**. Only the harmful-minus-benign gap carries information, and that gap is what gets reported per rung.
