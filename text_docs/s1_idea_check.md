...
**Adjacent-axis sub-check** (added after the external idea check, §12). Two further papers show a related shape on *other* axes and must be distinguished on first mention, neither a scoop:
- **Self-Jailbreaking** (Yong & Bach, ICLR 2026, arXiv 2510.20956) — after benign reasoning training, models stay internally aware of harmfulness yet reason themselves into compliance; mitigated by mixing in a small amount of safety-reasoning data. This is our A-failure pattern on the *reasoning-training* axis, and its fix has the same shape as H3. Converging evidence that the recognition/action split generalizes beyond one axis; differs from us in cause (training-induced drift, not input surface form), and runs no held-out-family generalization test and no can't-decode negative control.
- **DeRTa** (Yuan et al., ACL 2025, arXiv 2407.09121) — refusal-position bias: safety tuning teaches models to check at response *onset*, so refusal rarely fires later in a generation. Mechanistically load-bearing here: under encodings the harmful gist may only become legible mid-decode, so a model with intact recognition may have no trained pathway to refuse at that point. Supports the trajectory readouts in §7 and is a natural repair-side baseline/component.
...
- **H3 (repair generalizes).** Cross-encoding safety fine-tuning on a subset of families closes the action gap on *held-out* families, and the amount of generalization tracks **representational overlap** between families. *The overlap metric is pre-registered as the projection-score distribution overlap* (how far each family's harmfulness projections sit from the refusal threshold), **not** direction cosine: Wang et al. (NeurIPS 2025, arXiv 2505.17306) show refusal directions are near-parallel *across languages* — universal enough that an English-extracted vector bypasses refusal elsewhere — and Aziz et al. find the low-resource failure is a threshold/projection shift with the direction itself intact. If the same universality holds across encodings, direction cosine is degenerate (all ≈ 1) and has no variance to correlate with. Cosine is retained as a secondary check, and a *measured* near-degeneracy is itself a reportable result (it would say encodings shift where a family lands along a shared axis rather than rotating the axis). *Refuted if* held-out generalization is absent, or unrelated to every overlap metric measured.
...
- against **ACTOR** (Dabas et al., ICML 2025, arXiv 2507.04250), which repairs the *opposite* miscalibration (over-refusal) by shifting activations in a single layer. It is the cheapest known member of the fix class our story predicts should work — if a one-layer activation shift at the harmfulness-readout layer closes the action gap as well as LoRA SFT does, that is direct support for H5 (re-binding, not re-learning), and if it does not, the gap tells us how much of the repair is genuinely representational.
- against **DeRTa**-style late-refusal training (Yuan et al., ACL 2025) where the diagnosis shows recognition arriving mid-generation rather than at the instruction — the encoding analogue of refusal-position bias, and a fix that targets *when* refusal can fire rather than *what* it binds to.
...
Utility and over-refusal tracked throughout (a capability/MMLU slice + XSTest, plus **EVOREFUSE-TEST** (arXiv 2505.23473) as the harder over-refusal probe — it triggers ~85% more refusals than the next-best benchmark, and a cross-encoding safety fine-tune is exactly the kind of intervention that could buy safety by making the model refuse anything that looks encoded) so any fix is shown not to break the model.
...
- **Risk: representational overlap is degenerate across families.** If harmfulness directions are as universal across encodings as refusal directions are across languages (Wang et al. 2025), every family's direction cosine sits near 1 and H3's correlational half has no variance to work with. *Contingency:* the pre-registered overlap metric is projection-score distribution overlap, not direction cosine (§6, H3), which retains variance precisely in the regime universality predicts; decode-capability score is a second candidate predictor. Measured degeneracy is reported as a finding — it would say encodings move a family *along* a shared harmfulness axis rather than rotating it, which strengthens rather than weakens the binding story.
...
**Already covered** by the S1 corpus: Zhao et al. 2025 (`zhao2025llmsencode`, its top hit at 82%) and Arditi et al. 2024 (`arditi2024refusal`) — both already load-bearing in §7. That both surfaced at the top is a sanity check that the submitted framing reads as intended.

**Five papers newly actioned** (arXiv IDs verified against the primary source, not the tool's text):

| Paper | ID / venue | Disposition |
|---|---|---|
| Wang et al., *Refusal Direction is Universal Across Safety-Aligned Languages* | 2505.17306, NeurIPS 2025 | **Changed a hypothesis.** Refusal directions are near-parallel across languages (an English vector bypasses refusal elsewhere). If that universality extends to encodings, H3's direction-cosine predictor is degenerate. H3 now pre-registers projection-score distribution overlap as the primary metric (§6), with the degeneracy risk in §10. Was already in the science-repo intake queue as PolyRefuse's source; promoted to must-cite. |
| Yong & Bach, *Self-Jailbreaking* | 2510.20956, ICLR 2026 | **Must-distinguish + converging evidence** (§5): the A-failure shape on the reasoning-training axis, with a fix of the same shape as H3. |
| Yuan et al., *DeRTa* (Decoupled Refusal Training) | 2407.09121, ACL 2025 | **Mechanism + baseline** (§5, §7): refusal-position bias explains why a model that decodes harm mid-generation may have no trained pathway to refuse there. |
| Dabas et al., *Just Enough Shifts* (ACTOR) | 2507.04250, ICML 2025 | **Phase-3 baseline** (§7): single-layer activation-shift repair of the *opposite* miscalibration; the cheapest test of H5's re-binding claim. |
| Wu et al., *EVOREFUSE* | 2505.23473, NeurIPS 2025 | **Eval addition** (§7): harder over-refusal probe than XSTest alone. |

**Two "gaps" it named, both deliberately out of scope.** (a) *Multimodal encodings* (visual steganography, audio perturbations) — owned by the sibling repo `llm_guardrail_security` per the CLAUDE.md scope boundary; a future-work line, not this paper. (b) *Formal tamper-resistance guarantees* — no mechanistic paper offers them and this one will not either; it belongs in the limitations section, stated plainly.

Ingestion of the five into the science-repo corpus (`literature/model-internals/`) rides the existing intake queue.
