# AS-6 design notes — PREMATURE, PARKED (written 2026-08-05)

> ⚠️ **This is S6 material written at S1, out of workflow order. It is NOT the current stage and NOT ratified.**
>
> AS-6 is at **S1 — external idea check** (`s1_idea_check.md` in this directory), which is an owner-hands cspaper gate. The workflow's order is S1 idea check → S2 doability + home → S3 init → S4 literature → S5 main story → **S6 experiment design**. Everything below belongs to S6 and must not be treated as settled before the story exists: designing instruments before the story is fixed is exactly how a design gets built for a claim the project ends up not making.
>
> Kept rather than deleted because the content is grounded in measurements already made — the Gamma-Guard layer-27 siting, the verified feasibility spike, and the instrument law AS-5 paid for — and re-deriving it later would be waste. **Re-read and revise at S6; do not cite it before then.**

**Scope of record** is `text_docs/as5/proposal.md` §"AS-6 — the second paper" — that section holds the object cut, the delta statement, and the full prior-art record.
