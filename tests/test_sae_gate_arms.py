...
from internals_safety.measurements.sae_reconstruction import ReconstructionQuality, reading
...
class TestTheTargetArm:
    def test_it_is_judged_as_a_fraction_of_the_ceiling(self):
        record = reading(quality(0.60), CONFIG, ceiling=BASE_VE)
    ...
    def test_below_the_transfer_floor_refuses(self):
        assert reading(quality(0.50), CONFIG, ceiling=BASE_VE).licensed is False
    ...
    def test_the_floor_MOVES_with_the_ceiling(self):
        ...
        low = reading(quality(0.30), CONFIG, ceiling=0.35)
        high = reading(quality(0.30), CONFIG, ceiling=0.90)
...
class TestNeitherArmPassesWithoutAControl:
    ...
    @pytest.mark.parametrize("ceiling", [None, BASE_VE])
...
class TestTheReadingSaysWhichArmItIs:
    def test_a_record_can_always_tell_whether_it_SET_a_bar_or_was_measured_against_one(self):
        ...
        assert "TARGET arm" in reading(quality(0.6), CONFIG, ceiling=BASE_VE).operating_point
